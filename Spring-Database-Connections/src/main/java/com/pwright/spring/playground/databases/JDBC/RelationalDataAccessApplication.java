package com.pwright.spring.playground.databases.JDBC;
...
public class RelationalDataAccessApplication implements CommandLineRunner {
	...
	public void run(String... strings) throws Exception {
		...
		log.info("Querying for customer records where last_name = 'Dean':");
		jdbcTemplate.query("SELECT id, first_name, last_name FROM customers WHERE last_name = ?", 
				(rs, rowNum) -> new Customer(rs.getLong("id"), rs.getString("first_name"), rs.getString("last_name")),"Dean")
		.forEach(customer -> log.info(customer.toString()));
	}
